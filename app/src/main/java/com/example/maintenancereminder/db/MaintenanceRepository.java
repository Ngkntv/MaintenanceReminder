...
public class MaintenanceRepository {
    ...
    public void completeTask(Context context, MaintenanceTask task, long completionDate,
                             String comment, String consumables, Double cost) {
        if (task == null || task.id == null || task.deviceId == null
                || task.intervalValue == null || task.intervalUnit == null
                || task.nextDueDate == null) {
            throw new IllegalArgumentException("Invalid task in completeTask");
        }
    ...
    public boolean rollbackLastCompletion(Context context, long taskId) {
        ...
        try {
            ...
            Log.e(TAG, "rollbackLastCompletion failed", e);
            throw e;
        } finally {
            db.endTransaction();
        }
